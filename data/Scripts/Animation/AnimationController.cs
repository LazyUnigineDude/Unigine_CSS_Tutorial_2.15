...
[Component(PropertyGuid = "f16d4b005dddc25f576aac32fe579de97b8dce34")]
	...
	void AnimChanger(int SHOOTERSTATE)
	{
		...
		switch (STATE)
		{
			...
			case ANIM_STATE.WALK:
				...
				if (Input.IsKeyPressed(Input.KEY.SHIFT)) { ResetWeight(); STATE = ANIM_STATE.RUN; PREV_STATE = ANIM_STATE.WALK; }
			...
			case ANIM_STATE.REVERSE_WALK:
				...
				break;
			case ANIM_STATE.RUN:
				if (Input.IsKeyUp(Input.KEY.SHIFT)) { ResetWeight(); STATE = ANIM_STATE.WALK; PREV_STATE = ANIM_STATE.RUN; }
